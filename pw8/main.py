...
if check==True:
    ...
    t1 =threading.Thread(target=pickle.dump, args=(List_student,open('students.txt','wb')))
    t2 =threading.Thread(target=pickle.dump, args=(List_class,open('courses.txt','wb')))
    t3 =threading.Thread(target=pickle.dump, args=(Grade,open('marks.txt','wb')))
    ...
    sort_student()
    pickle.dump(idk.input.sorted_student,open('sorted.txt','wb'))
